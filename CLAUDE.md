...
**`/td-mailbox` is the unified cross-repo view.** One command walks both directions: inbound (issues filed INTO this repo, grouped by Issue Type) and outbound (cross-repo children of any parent in this repo). Per-item walk: close/comment/skip for inbound, comment/verify/reopen/skip for outbound. Single end-summary, no separate inbox vs outbox commands.

**Outbound tracking via sub-issues.** Every cross-repo issue this project files becomes a sub-issue of a parent in this repo — either an existing Epic if the work belongs to one, or the auto-created **outbound tracker Epic** (body opens with the `<!-- td-mailbox-tracker -->` sentinel + a "do not close" note; auto-created on first orphan cross-repo filing). The aggregate of all parents' cross-repo children IS the outbound set — one query, exact membership, no org-wide search. The `**From:** <project>` body marker still goes on every cross-repo filing — different role now: it's the human-readable identifier in non-GraphQL surfaces (`gh issue view`, etc.) and gives the receiver a stable source signal independent of the GH account that opened it. Sub-issue linkage is the canonical sender-side reference; the body marker is for humans.
...
- "file an issue for X" / "ask X to do Y" / "send a CR to X" → check `.td/PROJECT.md § Cross-repo`, then `gh api graphql createIssue` mutation against the target repo with body opening `**From:** <friendly-name>` followed by ask + why. (No `**Source:**` line — the sub-issue parent linkage created in the next step IS the sender-side reference.) Use the `Bug`/`Task`/`Idea` type that fits. **Then attach as sub-issue** (this is what makes it show up in `/td-mailbox` outbound): if the work belongs to an existing Epic in this repo, `addSubIssue` to that Epic; otherwise `addSubIssue` to the outbound tracker Epic (auto-create with the `<!-- td-mailbox-tracker -->` body sentinel if no tracker yet exists in this repo).
...
- `/td-mailbox` — unified cross-repo work check. Walks both directions in one pass: inbound (open GH issues filed INTO this repo, grouped by Issue Type with Epic sub-issue progress) and outbound (cross-repo children of any parent in this repo — Epic sub-issues + outbound tracker children, found via aggregate sub-issues query). One-at-a-time walk: close/comment/skip inbound, comment/verify/reopen/skip outbound. Single end-summary. Replaces the previous `/td-inbox` + `/td-outbox` split.
