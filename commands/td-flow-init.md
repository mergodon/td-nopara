...
Source templates from `~/projects/td-flow/templates/`. State-dir scaffolds live at `~/projects/td-flow/templates/td-flow/`:

- `CLAUDE.md` → root: copy `~/projects/td-flow/templates/CLAUDE.md` (a one-line `@import` of the canonical contract). It imports `~/.claude/td-flow-contract.md` — confirm that exists; if it doesn't, td-flow isn't fully installed, so tell the user to run `~/projects/td-flow/install.sh`, then continue.
- `mkdir -p .td-flow/work .td-flow/frameworks` — create the state dir + subdirs
- `.td-flow/PROJECT.md` → copy from `~/projects/td-flow/templates/td-flow/PROJECT.md`, fill placeholders
- `.td-flow/WORKWAY.md` → copy from `~/projects/td-flow/templates/td-flow/WORKWAY.md`, fill placeholders for Local testing, Local UAT, Live
- `.td-flow/STATE.md` → copy from `~/projects/td-flow/templates/td-flow/STATE.md`, fill placeholders, set `Last:` to today
- `.td-flow/BACKLOG.md` → copy from `~/projects/td-flow/templates/td-flow/BACKLOG.md` as-is
...
- **Laravel + Boost** detected:
  ...
  - If Boost overwrites root `CLAUDE.md`: restore the one-line `@import` (from `~/projects/td-flow/templates/CLAUDE.md`) and tell the user.
  ```
