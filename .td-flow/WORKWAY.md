...
- Pre-ship checks (automated by `scripts/smoke.sh`, OK/WARN/FAIL output, exit 0/1/2):
  ...
  - [x] **Cross-reference** (v7.1): every `commands/td-flow-*.md` listed in `EXPECTED_COMMANDS` also appears in `CLAUDE.md`'s `## The slash commands` trigger map, README's install symlinks list, AND README's slash commands table
