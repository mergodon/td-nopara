...
Phase:    6/7 — conversational "park" / "plan" / "flush" handlers + cross-repo Epic doc (2026-05-19)
Blocker:  none
Last:     2026-05-19 — **Commit 6/7 shipped: conversational handlers + cross-repo Epic doc.** No new slash command — three new entries in `CLAUDE.md § Where things go`: (1) "park this to GH" / "create an issue for X" → direct GH creation via `gh api graphql createIssue` with chosen Type ID, `**From:** <this-project>` marker, dedupe; (2) "flush the backlog" / "park the backlog to GH" → invokes /td-park inline; (3) "let's plan X" / "start an Epic for X" → work/<slug>.md as planning scratch, promotes to parent Epic via GraphQL when plan is solid, breaks out sub-issues via `addSubIssue` mutation (cross-repo within mergodon org supported). Also: new "Cross-repo Epics" paragraph in `§ Cross-repo` documenting the parent-Epic-with-sub-issues-in-other-repos pattern, with `$TD_REGISTRY` as the natural home for cross-project Epics and per-project Epics in their own repos. Cross-org parent-child unsupported. Updated existing "file an issue for X" entry to use `gh api graphql createIssue` (consistent with new pattern, supports Type ID attachment). Mirrored to templates/CLAUDE.md. **Plan ahead:** 7/7 = optional `DEBUG.md` doc + /td-incident integration. Earlier today: 1/7 = `9ee101d`; 2/7 = `522c623`; 3/7 = `6d767ee`; 4/7 = `3e64c6e`; 5/7 = `49c55d5`.
