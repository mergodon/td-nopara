...
Phase:    5/7 — /td-close adds park-leftovers first writer-side step (2026-05-19)
Blocker:  none
Last:     2026-05-19 — **Commit 5/7 shipped: `/td-close` gets `Step 3 — Park leftovers`.** Major restructure: /td-close now starts with park-leftovers as its first writer-side step. Walks (a) BACKLOG.md line by line using the same /td-park procedure, then (b) every .td/work/<topic>.md file — for each work file: check git log for shipped commits (delete if shipped) or ask ship-now/park-to-GH-as-Type/drop. Shipped via `gh api graphql createIssue` with type ID attached + `**From:** <sender-name>` marker. Old steps 7 (prune work/) and 8 (prune BACKLOG) folded into the new Step 3 — they were doing the same destruction less safely. Total /td-close steps reduced from 12 to 11. Cross-refs updated: "Steps 5-9" → "Steps 6-8" (the doc-audit window). After Step 3, BACKLOG is `(empty)` placeholder and work/ is clean; the rest of /td-close does code sanity + doc validation + final commit. The `Park leftovers` first-step framing reinforces the gh-source-of-truth model: parked thinking lives on GitHub, not in local docs. **Plan ahead:** 6/7 = conversational "park this" + "plan X" handlers (cross-repo sub-issues via td-registry); 7/7 = optional `DEBUG.md`. Earlier today: 1/7 = `9ee101d`; 2/7 = `522c623`; 3/7 = `6d767ee`; 4/7 = `3e64c6e`.
