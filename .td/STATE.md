...
Phase:    refined type heuristic (vague → Idea) + /td-refresh Phase 2 BACKLOG migration (2026-05-19)
Blocker:  none
Last:     2026-05-19 — Two refinements after the migration. (1) **Type-routing heuristic strengthened**: vague phrasing now defaults to `Idea`, not `Task`. Concrete verb + specific target ("remove unused imports in auth.ts") → Task; hedged or open-ended ("remove all the legacy stuff somehow") → Idea. Suggestion + the matching trigger phrase shown to the user for verification. Applies in /td-park Step 4, /td-close Step 3 (which delegates), and the conversational "park this" handler. (2) **/td-refresh gains Phase 2 — BACKLOG migration**: after the existing CLAUDE.md diff (Steps 0-6), a new Step 7 checks for accumulated BACKLOG items and offers to flush via the /td-park procedure. Step 8 wraps with a "Refresh complete" summary covering both phases. Skips silently when BACKLOG is empty. Migration path for projects that pre-date the gh-source-of-truth model. CLAUDE.md /td-refresh description updated to reflect both phases; templates/CLAUDE.md mirrored. Earlier today: README rewrite (3031639); gh-source-of-truth migration commits 9ee101d / 522c623 / 6d767ee / 3e64c6e / 49c55d5 / 8b3b462 / 6b1236c. Open follow-ups: land NAMING.md in td-registry; fold-and-delete that scratch; opportunistic repo renames per the convention.
