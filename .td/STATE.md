...
Phase:    7/7 — DEBUG.md optional canonical doc + template + routing (2026-05-19) — MIGRATION COMPLETE
Blocker:  none
Last:     2026-05-19 — **Commit 7/7 shipped: `DEBUG.md` as optional canonical doc — gh-source-of-truth migration COMPLETE.** Added `templates/td/DEBUG.md` template with structure: Tooling section (logs/errors/metrics/hosting/DB), Symptom→diagnostic path entries, Gotchas (symptom/root-cause/workaround), Production debug commands, "When to add" guidance. Opt-in per project (same pattern as `PROJECT.md § Cross-repo`) — not scaffolded at `/td-init`, created on demand. CLAUDE.md `§ The docs` documents DEBUG.md as an optional sixth doc; routing map adds "add to DEBUG" / "save this debug trick" trigger that writes to `.td/DEBUG.md` and creates from template if missing. `/td-incident` already references DEBUG.md (surfaces if present in Step 4, asks "save this to DEBUG.md?" in resolution path a). **All 7 commits shipped today:** 1/7 `9ee101d` (BACKLOG session-scoped); 2/7 `522c623` (/td-incident); 3/7 `6d767ee` (/td-inbox GraphQL); 4/7 `3e64c6e` (/td-park); 5/7 `49c55d5` (/td-close park-leftovers); 6/7 `8b3b462` (conversational handlers + cross-repo Epic); 7/7 this commit. **Framework state:** 7 slash commands (init/clear/close/refresh/inbox/incident/park), 5 core docs + 1 optional (PROJECT/WORKWAY/STATE/BACKLOG/work + DEBUG), 5 Issue Types at mergodon org (Idea/Task/Bug/Feature/Epic), full GitHub-as-source-of-truth model with cross-repo sub-issues for Epics. Open follow-ups: land NAMING.md in td-registry (`.td/work/naming-convention.md` still pending); fold-and-delete that scratch when landed; opportunistic repo renames per the convention.
