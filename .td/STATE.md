...
Phase:    3/7 — /td-inbox GraphQL rewrite (Issue Type grouping + sub-issue progress) (2026-05-19)
Blocker:  none
Last:     2026-05-19 — **Commit 3/7 shipped: `/td-inbox` rewritten on GraphQL.** Switched listing from `gh issue list` to `gh api graphql` (with `GraphQL-Features: sub_issues` header) to fetch each issue's `issueType` field and `subIssuesSummary { total, completed, percentCompleted }`. Output now grouped by type — Epic first (with sub-issue progress bar like `[3/8 closed, 37%]`), then Bug, Feature, Task, Idea, then any untyped. Within bucket, sorted by updatedAt desc. Type-aware recommendations: Epic at 100% sub-issues → close parent; Epic stale at 0% → still pursuing?; Idea > 60 days → close?; etc. Standard close/comment/skip per issue retained, sign-off unchanged. Degraded fallback: if GraphQL schema drift breaks the preview field, falls back to `gh issue list --json` without type grouping. **Plan ahead:** 4/7 = `/td-park`; 5/7 = `/td-close` park-leftovers; 6/7 = conversational handlers; 7/7 = optional `DEBUG.md`. Earlier commits today: 1/7 = `9ee101d` (BACKLOG session-scoped); 2/7 = `522c623` (/td-incident).
