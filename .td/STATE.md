...
Phase:    2/7 — /td-incident slash command (2026-05-19)
Blocker:  none
Last:     2026-05-19 — **Commit 2/7 shipped: `/td-incident` slash command.** Live production fire mode — drops everything else, asks one-liner of what's broken, sets STATE to incident, opens `.td/work/incident-<slug>.md`, surfaces `DEBUG.md` if present (graceful fallback if not), walks diagnosis with read-only-by-default constraint, resolves three ways: fix-now (commit `fix(<area>):` + fold-and-delete work file + optional DEBUG.md capture), park to GH as `Bug` (creates issue via `gh api graphql` with Type attached), or file cross-repo (per existing `CLAUDE.md § Cross-repo` flow with `From:` marker + Type `Bug`). 6 slash commands now (`init`, `clear`, `close`, `refresh`, `inbox`, `incident`); install.sh re-run confirms all symlinked. **Plan ahead:** Commit 3/7 = `/td-inbox` GraphQL rewrite with Issue Type grouping + sub-issue progress; 4/7 = `/td-park`; 5/7 = `/td-close` park-leftovers step; 6/7 = conversational handlers (park-this + plan-X with cross-repo sub-issues); 7/7 = optional `DEBUG.md` doc. Commit 1/7 (BACKLOG session-scoped semantics) shipped earlier today as `9ee101d`.
